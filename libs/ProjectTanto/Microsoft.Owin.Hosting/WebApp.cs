...
namespace Microsoft.Owin.Hosting
{
    ...
    public static class WebApp
    {
        ...
        private static IDisposable StartImplementation(IServiceProvider services, StartOptions options)
        {
            ...
            if (starter == null)
            {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, "Resources.Exception_FailedToResolveService", "IHostingStarter"));
            }
        ...
        private static IDisposable StartImplementation(IServiceProvider services, StartOptions options, Action<IAppBuilder> startup)
        {
            ...
            if (string.IsNullOrWhiteSpace(options.AppStartup))
            {
                ...
                options.AppStartup = startup.GetMethodInfo().DeclaringType.FullName; //ReflectedType
            }

            var engine = services.GetService<IHostingEngine>();
            if (engine == null)
            {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, "Resources.Exception_FailedToResolveService", "IHostingEngine"));
            }
