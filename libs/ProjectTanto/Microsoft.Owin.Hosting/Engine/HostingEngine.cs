...
namespace Microsoft.Owin.Hosting.Engine
{
    ...
    public class HostingEngine : IHostingEngine
    {
        ...
        private readonly IAppBuilderFactory appBuilderFactory;
        //private readonly ITraceOutputFactory traceOutputFactory;
        private readonly IAppLoader appLoader;
        private readonly IServerFactoryLoader serverFactoryLoader;
       // private readonly ILoggerFactory loggerFactory;
           ...
           )
        {
            ...
            this.appBuilderFactory = appBuilderFactory;
            //this.traceOutputFactory = traceOutputFactory;
            this.appLoader = appLoader;
            this.serverFactoryLoader = serverFactoryLoader;
        ...
        private void ResolveOutput(StartContext context)
        {
            if (context.TraceOutput == null)
            {
                ...
                //context.TraceOutput = traceOutputFactory.Create(traceoutput);
            }
        ...
        private void InitializeBuilder(StartContext context)
        {
            ...
            {
                context.Builder = appBuilderFactory.Create();
            }
            ...
            out string path)
        {
            ...
            int delimiterStart1 = url.IndexOf(SchemeDelimiter, StringComparison.Ordinal);
            ...
            int delimiterEnd1 = delimiterStart1 + SchemeDelimiter.Length;
            ...
            else
            {
                ...
                if (string.Equals(scheme, UriSchemeHttp, StringComparison.OrdinalIgnoreCase))
                {
                    port = "80";
                }
                else if (string.Equals(scheme, UriSchemeHttps, StringComparison.OrdinalIgnoreCase))
                {
                    port = "443";
                }
                else
        ...
        private void EnableTracing(StartContext context)
        {
            ...
            //context.Builder.SetLoggerFactory(loggerFactory);
        }
        ...
        private void ResolveServerFactory(StartContext context)
        {
            ...
            context.ServerFactory = serverFactoryLoader.Load(serverName);
            if (context.ServerFactory == null)
            {
                throw new MissingMemberException(string.Format(CultureInfo.InvariantCulture, "Resources.Exception_ServerNotFound {0}", serverName));
            }
        }
        ...
        private void ResolveApp(StartContext context)
        {
            ...
            if (context.App == null)
            {
                ...
                if (context.Startup == null)
                {
                    ...
                    context.Startup = appLoader.Load(appName, errors);
                }
                if (context.Startup == null)
                {
                    throw new Exception("Resources.Exception_AppLoadFailure "
