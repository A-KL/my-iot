...
namespace Microsoft.Owin.Builder
{
    ...
    public class AppBuilder : IAppBuilder
    {
        ...
        private readonly IList<Tuple<Type, Delegate, object[]>> middleware;
        private readonly IDictionary<Tuple<Type, Type>, Delegate> conversions;
        ...
        public AppBuilder()
        {
            ...
            this.conversions = new Dictionary<Tuple<Type, Type>, Delegate>();
            this.middleware = new List<Tuple<Type, Delegate, object[]>>();

            this.properties[Constants.BuilderAddConversion] = new Action<Delegate>(AddSignatureConversion);
            this.properties[Constants.BuilderDefaultApp] = NotFound;
        ...
        {
            this.middleware.Add(ToMiddlewareFactory(middleWare, args));
            return this;
        }
        ...
        {
            return new AppBuilder(conversions, properties);
        }
        ...
        private void AddSignatureConversion(Delegate conversion)
        {
            ...
            var key = Tuple.Create(conversion.GetMethodInfo().ReturnType, parameterType);
        ...
        {
            object app;
            if (!properties.TryGetValue(Constants.BuilderDefaultApp, out app))
            {
                app = NotFound;
            }

            foreach (var middle in middleware.Reverse())
            {
                var neededSignature = middle.Item1;
                var middlewareDelegate = middle.Item2;
                var middlewareArgs = middle.Item3;
        ...
        private object Convert(Type signature, object app)
        {
            ...
            throw new ArgumentException(
                string.Format(CultureInfo.CurrentCulture, "Resources.Exception_NoConversionExists {0} {1}", app.GetType(), signature),
                "signature");
        }
        ...
        {
            var signatureMethod = signature.GetMethod(Constants.Invoke);
        ...
        private static Tuple<Type, Delegate, object[]> ToMiddlewareFactory(object middlewareObject, object[] args)
        {
            ...
            var factory = ToInstanceMiddlewareFactory(middlewareObject, args);
            ...
            throw new NotSupportedException(
                string.Format(
                    ...
                    "Resources.Exception_MiddlewareNotSupported {0}",
        ...
        private static Tuple<Type, Delegate, object[]> ToInstanceMiddlewareFactory(object middlewareObject, object[] args)
        {
            ...
            foreach (var method in methods)
            {
                ...
                {
                    var invokeParameters = new[] { app }.Concat(args).ToArray();
        ...
        private static Tuple<Type, Delegate, object[]> ToGeneratorMiddlewareFactory(object middlewareObject, object[] args)
        {
            ...
            foreach (var method in methods)
            {
                ...
                var middlewareDelegate = method.CreateDelegate(funcType, middlewareObject); // Delegate.CreateDelegate(funcType, middlewareObject, method);
        ...
        private static Tuple<Type, Delegate, object[]> ToConstructorMiddlewareFactory(object middlewareObject, object[] args, ref Delegate middlewareDelegate)
        {
            ...
            throw new MissingMethodException(
                ...
                "Resources.Exception_NoConstructorFound {0} {1}",
        ...
        private static bool TestArgForParameter(Type parameterType, object arg)
        {
            ...
            return (arg == null) || parameterType.IsInstanceOfType(arg);
        }
    }
}
